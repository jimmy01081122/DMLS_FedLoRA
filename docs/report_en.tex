...
\section{Discussion}
The results highlight a clear Pareto front where RoLoRA provides an optimal balance between communication cost and model capacity, particularly in heterogeneous environments. The robustness to client dropout was observed across all methods, but the single-sided update variants (FFA and RoLoRA) appeared more resilient to partial information loss, likely due to their reduced parameter search space per round.

The significant drop in performance at $\alpha=0.1$ suggests that while PEFT methods are efficient, they require higher samples or more sophisticated local optimization to handle extreme label skew in federated settings. Furthermore, the 4-bit quantization did not hinder the ability to observe these algorithmic differences, confirming the viability of the RTX 3050 for high-fidelity federated learning research.
